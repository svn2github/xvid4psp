...
DEFINE_GUID(CLSID_VideoSink,
0xf13d3732, 0x96bd, 0x4108, 0xaf, 0xeb, 0xe8, 0x5f, 0x68, 0xff, 0x64, 0xdc);

const char  *g_module_name = "AVSS";
...
class DSS2 : public IClip {
  ...
  HRESULT Open(const wchar_t *filename, long long avgf) {
    ...
    if (pPB)
      pPB->Write(L"ui.interactive", &CComVariant(0u, VT_UI4));

    CComPtr<IPin>	    pO(GetPin(pS, false, PINDIR_OUTPUT, &MEDIATYPE_Video));
    if (!pO)
      pO = GetPin(pS, false, PINDIR_OUTPUT, &MEDIATYPE_Stream);
  ...
  static void ReadFrame(long long timestamp, unsigned format, unsigned bpp,
...
public:
  ...
  PVideoFrame __stdcall       GetFrame(int n, IScriptEnvironment *env) {
    ...
    if (n < m_qfirst || n > m_qnext + m_seek_thr) { // we need to seek to get this frame
    ...
    for (;;) {
      ...
      if (frameno < m_qnext) // preroll frame
        continue;

      if (frameno == m_qnext) { // we want this frame, compare timestamps to account for decimation
      ...
      } else { // somehow we got beyond the place we wanted, fill in intermediate frames
