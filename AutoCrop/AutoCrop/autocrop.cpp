...
class AutoCrop : public GenericVideoFilter {
		...
		bool overwrite;
public:
    AutoCrop(PClip _child, int _mode, int _wMultOf, int _hMultOf, int _leftAdd, int _topAdd, int _rightAdd, int _bottomAdd, int _threshold, int _samples, int _samplesstart, int _samplesend, float _aspect, const char* _file, bool _overwrite, IScriptEnvironment *env) : GenericVideoFilter(_child) {
...
int autocropLog(char *str, char const* str2, bool overwrite)
{
	char buf[4096];
	FILE *fp=NULL;
	sprintf(buf,"%s\n",str);
	if (overwrite){ 
		fp = fopen(str2,"w"); //overwrite
	}
	else { 
		fp = fopen(str2,"a"); //append
	}
	if (fp) {
		fwrite(buf,strlen(buf),1,fp);
		fclose(fp);
