...
int autocropLog(char *str, const char* str2);////
...
class AutoCrop : public GenericVideoFilter {
        ...
        const char* file;
public:
    AutoCrop(PClip _child, int _mode, int _wMultOf, int _hMultOf, int _leftAdd, int _topAdd, int _rightAdd, int _bottomAdd, int _threshold, int _samples, int _samplesstart, int _samplesend, float _aspect, const char* _file, IScriptEnvironment *env) : GenericVideoFilter(_child) {
			xMultipleOf = __min(_wMultOf,vi.width);
			yMultipleOf = __min(_hMultOf,vi.height);
			if (vi.IsYUY2()) {
...
int __stdcall AutoCrop::SampleFrames(IScriptEnvironment* env)
{
    PVideoFrame eval;
		...
		right = __min(eval->GetRowSize(),right-rightAdd);
		top = __max(0,top+topAdd);
		bottom = __min(eval->GetHeight(),bottom-bottomAdd);
...
extern "C" __declspec(dllexport) const char* __stdcall AvisynthPluginInit2(IScriptEnvironment* env) {
    env->AddFunction("AutoCrop", "c[mode]i[wMultOf]i[hMultOf]i[leftAdd]i[topAdd]i[rightAdd]i[bottomAdd]i[threshold]i[samples]i[samplestartframe]i[sampleendframe]i[aspect]f[file]s", Create_AutoCrop, 0);
...
int autocropLog(char *str, char const* str2)
{
