    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                Console.InputEncoding = Encoding.GetEncoding(CultureInfo.CurrentCulture.TextInfo.OEMCodePage);
                ...
                if (args.Length > 0)
                {
                    ...
                    //диалог открытия видео файлов
                    if (type.Substring(0, 2) == "ov")
                    {
                        ...
                        o.Filter = phrases[0] + "|*.avi;*.divx;*.wmv;*.mpg;*.mpe;*.mpeg;*.mod;*.asf;*.mkv;*.mov;*.qt;*.3gp;*.hdmov;*.mp4;*.ogm;*.avs;*.vob;*.dvr-ms;*.ts;*.m2p;*.m2t;*.m2v;*.d2v;*.m2ts;*.rm;*.ram;*.rmvb;*.rpx;*.smi;*.smil;*.flv;*.pmp;*.h264;*.264;*.vro|" +
                    ...
                    //диалог открытия аудио файлов
                    if (type.Substring(0, 2) == "oa")
                    {
                        //быстро переводим фразы
                        ArrayList phrases = new ArrayList();
                    ...
                    if (type == "mkv")
                    {
                        ...
                        OpenFileDialog o = new OpenFileDialog();
                        o.Filter = "Matroska " + phrases[1] + " (MKV)|*.mkv|" +
                    phrases[2] + " (*.*)|*.*";
                        o.Multiselect = false;
                        o.Title = phrases[3] + ":";
                    ...
                    if (type == "sub")
                    {
                        ...
                        OpenFileDialog o = new OpenFileDialog();
                        o.Filter = phrases[0] + "|*.srt;*.sub;*.idx;*.ssa;*.ass;*.psb;*.smi|" +
                        phrases[1] + " (*.*)|*.*";
                        o.Multiselect = false;
                        o.Title = phrases[2] + ":";
