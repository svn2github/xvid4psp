...
namespace XviD4PSP
{
    ...
    if(c->dia_size==-1)        funny_diamond_search
        ...
        public void LoadFromProfile()
        {
            ...
            combo_threads.SelectedIndex = m.ffmpeg_options.threads;
            ...
            combo_bframes.IsEnabled = !m.ffmpeg_options.extra_cli.Contains("-bf ");
            combo_bdecision.IsEnabled = !m.ffmpeg_options.extra_cli.Contains("-brd_scale ");
            combo_brefine.IsEnabled = !m.ffmpeg_options.extra_cli.Contains("-bidir_refine ");
            ...
            check_qpel.IsEnabled = !m.ffmpeg_options.extra_cli.Contains("+qpel");
            ...
            combo_threads.IsEnabled = !m.ffmpeg_options.extra_cli.Contains("-threads ");
        ...
        private void SetToolTips()
        {
            ...
            combo_bframes.ToolTip = "Set maximum number of B frames between non-B-frames (-bf, default: 0)";
            combo_bdecision.ToolTip = "Downscale frames for dynamic B-frame decision (-brd_scale, default: Disabled)";
            combo_brefine.ToolTip = "Refine the two motion vectors used in bidirectional macroblocks (-bidir_refine, default: 1)";
            ...
            combo_threads.ToolTip = "Set number of threads for encoding (-threads, default: Auto)";
        ...
        public static Massive EncodeLine(Massive m)
        {
            ...
            if (m.ffmpeg_options.bframes != defaults.bframes && !m.ffmpeg_options.extra_cli.Contains("-bf "))
                line += " -bf " + m.ffmpeg_options.bframes;

            if (m.ffmpeg_options.bdecision != defaults.bdecision && !m.ffmpeg_options.extra_cli.Contains("-brd_scale "))
                line += " -brd_scale " + ((m.ffmpeg_options.bdecision == "Disabled") ? "0" : m.ffmpeg_options.bdecision);

            if (m.ffmpeg_options.brefine != defaults.brefine && !m.ffmpeg_options.extra_cli.Contains("-bidir_refine "))
                line += " -bidir_refine " + ((m.ffmpeg_options.brefine == "Disabled") ? "0" : m.ffmpeg_options.brefine);
            ...
            if (m.ffmpeg_options.qpel && !defaults.qpel && !m.ffmpeg_options.extra_cli.Contains("+qpel"))
                flags += "+qpel";
            ...
            if (m.ffmpeg_options.threads != defaults.threads && !m.ffmpeg_options.extra_cli.Contains("-threads "))
                line += " -threads " + m.ffmpeg_options.threads;
        ...
        private void combo_codec_preset_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e)
        {
            if ((combo_codec_preset.IsDropDownOpen || combo_codec_preset.IsSelectionBoxHighlighted) && combo_codec_preset.SelectedItem != null)
            {
                ...
                else if (preset == CodecPresets.Ultra)
                {
                    ...
                    m.ffmpeg_options.mv0 = false;
                    m.ffmpeg_options.mv4 = true;
                    m.ffmpeg_options.qpel = Format.GetValidQPEL(m);
                    m.ffmpeg_options.qprd = false;
        ...
        private void DetectCodecPreset()
        {
            ...
            //Default
            if ((string.IsNullOrEmpty(m.ffmpeg_options.vcodec) || m.ffmpeg_options.vcodec == "mpeg4") &&
                m.ffmpeg_options.fourcc == "Default" &&
                m.ffmpeg_options.aic == false &&
                m.ffmpeg_options.bdecision == "Disabled" &&
                m.ffmpeg_options.bframes == 0 &&
        ...
        private void check_qpel_Click(object sender, RoutedEventArgs e)
        {
            m.ffmpeg_options.qpel = check_qpel.IsChecked.Value;
            root_window.UpdateManualProfile();
            DetectCodecPreset();
        }
        ...
        private void combo_bframes_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e)
        {
            ...
            {
                m.ffmpeg_options.bframes = combo_bframes.SelectedIndex;
        ...
        {
            if ((combo_bdecision.IsDropDownOpen || combo_bdecision.IsSelectionBoxHighlighted) && combo_bdecision.SelectedItem != null)
            {
        ...
        {
            if ((combo_brefine.IsDropDownOpen || combo_brefine.IsSelectionBoxHighlighted) && combo_brefine.SelectedItem != null)
            {
        ...
        private void combo_threads_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            ...
            {
                m.ffmpeg_options.threads = combo_threads.SelectedIndex;
                root_window.UpdateManualProfile();
                DetectCodecPreset();
            }
        }
        ...
        private void button_Apply_CLI_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                m.vencoding = "Custom MPEG4 CLI";    //- Изменяем название пресета           
                PresetLoader.CreateVProfile(m);      //- Перезаписываем файл пресета (m.vpasses[x])
                root_window.m = this.m.Clone();      //- Передаем массив в основное окно
                root_window.LoadProfiles();          //- Обновляем название выбранного пресета в основном окне (Custom MPEG4 CLI)
            }
            catch (Exception)
            {
